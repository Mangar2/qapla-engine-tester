...
QaplaHelpers::StableMap<std::string, ParameterDefinition> getEngineKeys() {
    return {
        ...
        { "dir",       { .description = "Working directory",
                        ...
                        .defaultValue = std::nullopt,
                        .type = ValueType::PathExists}},
        { "args",      { .description = "Command line arguments passed to the engine executable",
                        .isRequired = false,
                        .defaultValue = std::nullopt,
                        .type = ValueType::String } },
        { "proto",     { .description = "Protocol (uci/xboard)",
                        ...
                        .defaultValue = std::nullopt,
                        .type = ValueType::String } },
        { "tc",        { .description = "Time control in format moves/time+inc, or a fixed limit (depth:N, nodes:N, movetime(ms):N, mate:N, 'inf'). Each engine plays under its own tc, so engines may use different kinds", 
                        .isRequired = false, 
                        .defaultValue = std::nullopt, 
                        .type = ValueType::String } },
        ...
        { "gauntlet",  { .description = "Set if engine is part of the gauntlet group.",
                        ...
                        .defaultValue = false,
                        .type = ValueType::Bool } },
        // Reported scores are normally seen from the side to move. Kept defaultless and hidden:
        // it only needs to appear in a file if an engine actually deviates from that.
        { "whitepov",  { .description = "Engine reports scores from white's point of view",
                        .isRequired = false,
                        .defaultValue = std::nullopt,
                        .type = ValueType::Bool,
                        .isHidden = true } },
...
QaplaHelpers::StableMap<std::string, ParameterDefinition> getEpdKeys() {
    return {
        ...
        { "maxtime",   { .description = "Maximum allowed time in seconds per move during EPD analysis.", 
                        .longDescription = "Maximum allowed time in seconds per move during EPD analysis. Ignored when 'depth' or 'nodes' is greater than 0.",
                        .isRequired = false, 
                        .defaultValue = 20, 
                        .type = ValueType::UInt } },
        { "mintime",   { .description = "Minimum required time for an early stop, when a correct move is found", 
                        .longDescription = "Minimum required time for an early stop, when a correct move is found. Ignored when 'depth' or 'nodes' is greater than 0.",
                        .isRequired = false, 
                        .defaultValue = 2, 
                        .type = ValueType::UInt } },
        { "seenplies", { .description = "Amount of plies one of the expected moves must be shown to stop early (0 = off)", 
                        .longDescription = "Amount of plies one of the expected moves must be shown to stop early (0 = off). Ignored when 'depth' or 'nodes' is greater than 0.",
                        .isRequired = false, 
                        .defaultValue = 0, 
                        .type = ValueType::UInt } },
        { "depth",     { .description = "Fixed search depth per position (0 = off, overrides time settings)",
                        .longDescription = "Fixed search depth per position. If this value is greater than 0, EPD runs with a depth-limited search and ignores 'maxtime', 'mintime', and 'seenplies'. Value 0 means not set. Must not be combined with 'nodes' > 0.",
                        .isRequired = false,
                        .defaultValue = 0,
                        .type = ValueType::UInt } },
        { "nodes",     { .description = "Fixed node limit per position (0 = off, overrides time settings)",
                        .longDescription = "Fixed node limit per position. If this value is greater than 0, EPD runs with a node-limited search and ignores 'maxtime', 'mintime', and 'seenplies'. Value 0 means not set. Must not be combined with 'depth' > 0.",
                        .isRequired = false,
                        .defaultValue = 0,
                        .type = ValueType::UInt } },
...
QaplaHelpers::StableMap<std::string, ParameterDefinition> getTestKeys() {
    return {
        ...
        { "nomemory",   { .description = "Skip hash table memory usage test",
                        .longDescription = "Disables the hash table test, which verifies if the engine correctly allocates and frees memory when the 'Hash' option is changed. Off by default: relies on the OS returning freed memory to the process's RSS promptly, which macOS does not do, making the check unreliable there.",
                        .isRequired = false,
                        .defaultValue = true,
                        .type = ValueType::Bool } },
        { "nooption",   { .description = "Skip option crash tests",
                        ...
                        .defaultValue = false,
                        .type = ValueType::Bool } },
        { "nolowercase", { .description = "Skip lower-case option name test",
                        .longDescription = "Disables the test that checks whether the engine accepts lower-case UCI option names. Off by default: the check compares process RSS before/after, which macOS does not update reliably for this purpose.",
                        .isRequired = false,
                        .defaultValue = true,
                        .type = ValueType::Bool } },
