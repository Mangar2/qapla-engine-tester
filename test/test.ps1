$tests = @(
    ...
    ,@{ args = "--logpath=log --enginelog --pgnoutput file=log/test.pgn --each tc=10+0.02 --openings order=random file=book8ply.raw format=raw --tournament type=gauntlet games=8 repeat=2 --engine conf=""Qapla 0.3.2"" gauntlet --engine conf=""Qapla 0.3.2"" trace=all --engine conf=""Qapla 0.3.1"" --enginesfile=C:\Development\qapla-engine-tester\test\engines.ini"; expectedGode = 0 },
